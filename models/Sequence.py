...
class Sequence(db.Model):
	...
	def __repr__(self):
		return ('<Sequence: type=%s, start=%d, end=%d, current=%s, active=%s, '
			'created_at=%s>') % (self.type, self.start, self.end, self.current, self.active, self.created_at)
...
def init_sequence(typ, start, end):
	assert start < end
	root = Sequence.get_by_key_name('_%s_root' % typ)
	if not root:
		root = Sequence.get_or_insert('_%s_root' % typ, type='_root', start=0, end=1, active=False)
...
def get_numbers(typ, needed):
	...
	query = Sequence.all(keys_only=True).filter('type = ', typ).filter('active = ', True).order('start')
	rows = query.fetch(5)
	if rows:
